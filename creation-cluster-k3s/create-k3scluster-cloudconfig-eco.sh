...
export OPTIONS="--no-deploy traefik --cluster-cidr=192.168.0.0/16"
...
for node in node{1..3}; do echo "create node $node"; multipass launch focal --disk 15G -n $node --cloud-init cloud-config.yaml; done
